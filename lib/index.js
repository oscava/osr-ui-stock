(function(window) {
    ...
    var Line = Class.extends({
        ...
        onDraw: function(last, now, index) {
            if (!last) return;
            this.beginPath();
            var lx = this.offsetPosition((index - 1) * (this.screen.item.width + this.screen.item.padding * 2));
            var nx = this.offsetPosition(index * (this.screen.item.width + this.screen.item.padding * 2));
    ...
    var KLine = Line.extends({
        ...
        onDraw: function(last, now, index) {
            ...
            if (isBear) {
                this.context.strokeStyle = this._defaultStyle.bear.color;
                this.context.fillStyle = this._defaultStyle.bear.color;
            } else {
                this.context.strokeStyle = this._defaultStyle.bull.color;
                this.context.fillStyle = this._defaultStyle.bull.color;
            }
            var x = this.offsetPosition(index * (this.screen.item.width + this.screen.item.padding * 2));
            ...
            var lzx = this.offsetPosition(index * (this.screen.item.width + this.screen.item.padding * 2) - (this.screen.item.width / 2));
